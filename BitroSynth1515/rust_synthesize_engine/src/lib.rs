...
static DURATION_OF_LAST_SOUND : AtomicI32 = AtomicI32::new(0);
...
#[unsafe(no_mangle)]
pub extern "C" fn get_duration_of_last_sound() -> c_int {
    return DURATION_OF_LAST_SOUND.load(Ordering::SeqCst) as c_int;
}
...
fn write_wav(path: String, samples: &[f32], bit_8: c_uchar) -> c_int {
    // 1 = true
    // 0 = false
    let sample_rate : u32 = SAMPLE_RATE.load(Ordering::SeqCst) as u32;
    let duration_ms = (samples.len() as u64 * 1000 / sample_rate as u64) as i32;
    DURATION_OF_LAST_SOUND.store(duration_ms, Ordering::SeqCst);
